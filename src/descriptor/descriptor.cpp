...
std::unique_ptr<EngineDescriptorPool> EngineDescriptorPool::Builder::build() const {
  return std::make_unique<EngineDescriptorPool>(this->engineDevice, maxSets, poolFlags, poolSizes);
}
...
EngineDescriptorPool::EngineDescriptorPool(
  ...
  if (vkCreateDescriptorPool(this->engineDevice.getLogicalDevice(), &descriptorPoolInfo, nullptr, &descriptorPool) !=
      VK_SUCCESS) {
...
void EngineDescriptorPool::resetPool() {
  vkResetDescriptorPool(this->engineDevice.getLogicalDevice(), descriptorPool, 0);
}
...
void EngineDescriptorWriter::overwrite(VkDescriptorSet &set) {
  ...
  vkUpdateDescriptorSets(pool.engineDevice.getLogicalDevice(), writes.size(), writes.data(), 0, nullptr);
}
